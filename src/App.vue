          ...
          <b-row>
              ...
              <b-col>
                ...
                <b-col>
					  ...
					  <b-row>
                <b-col>
					  			<b-input-group prepend="Mailbody">
					  						<b-form-input type="text" v-model="editingItem.mailbody"></b-form-input>
					  			</b-input-group>
					  		</b-col>
					  </b-row>
					  <b-row>
                <b-col>
					  ...
					  <b-row>
                <b-col>
					  			<b-input-group prepend="Ref Title">
					  						<b-form-input type="text" v-model="editingItem.referenceName"></b-form-input>
					  			</b-input-group>
					  		</b-col>
					  </b-row>
					</template>
...
//				{key: 'name', sortable: true},
    ...
    getAllConf() {
			 var self = this
       axios.get('http://127.0.0.1:3000/getallconf').then(res => {
	  ...
	  save(item, index, button) {
			...
			if (!this.isValid(item)) {
			  ...
			  axios.post('http://127.0.0.1:3000/saveconf', {
			  	item: item,
			  	originItemName: self.originItemName
			  }).then(res => {
		...
		add() {
		  ...
		  axios.post('http://127.0.0.1:3000/addconf', {
		  	item: item,
		  }).then(res => {
		  	self.editingItem = {}
		  	self.postContent = ""
	  ...
	  copy(item, index, button) {
			...
			if (item.status.isrunning) {
			  ...
			  axios.post('http://127.0.0.1:3000/copyconf', {
			  	item: item
			  }).then(res => {
	  ...
	  remove(item, index, button) {
			...
			if (item.status.isrunning) {
			  ...
			  axios.post('http://127.0.0.1:3000/removeconf', {
			  	item: item
			  }).then(res => {
	  ...
	  test(item, index, button) {
			var self = this
			axios.post('http://127.0.0.1:3000/testconf', {
				item: item,
			}).then(res => {
	  ...
	  toggleStat(item, index, button) {
			...
			if (!item.status.passtest) {
			  ...
			  axios.post('http://127.0.0.1:3000/togglecron', {
			  	item: item
			  }).then(res => {
